    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {isAdmin && (
            <div className="flex flex-col sm:flex-row gap-2 pb-4 border-b border-slate-200 dark:border-slate-700">
              ...
              <select
                ...
                className="h-10 px-3 rounded-lg border-2 border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {ALL_ROLES.map((r) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="text-left py-2 px-3 font-medium text-slate-500 dark:text-slate-400">Email</th>
                    <th className="text-left py-2 px-3 font-medium text-slate-500 dark:text-slate-400">Role</th>
                    <th className="text-left py-2 px-3 font-medium text-slate-500 dark:text-slate-400">Joined</th>
                    {isAdmin && (
                      <th className="py-2 px-3 font-medium text-slate-500 dark:text-slate-400 w-16" />
                    )}
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td className="py-3 px-3">
                          {isAdmin && !isCurrentUser ? (
                            <select
                              ...
                              className="h-8 px-2 rounded-md border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                            >
                              {ALL_ROLES.map((r) => (
                                <option key={r} value={r}>{ROLE_LABELS[r]}</option>
                              ))}
                          ...
                          <td className="py-3 px-3 text-right">
                            {!isCurrentUser && (
                              <button
                                ...
                                className="p-1.5 text-slate-400 hover:text-red-500 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-red-500"
                              >
                                <Trash2 size={15} />
                              </button>
                            )}
