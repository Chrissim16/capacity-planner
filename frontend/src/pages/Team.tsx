...
export function Team() {
 ...
 <div className="flex items-center gap-2 px-4 py-2.5 bg-[#E6F2FC] border border-blue-200 rounded-lg text-sm text-blue-800">
 ...
 isActive ? tab.activeColor : 'border-transparent text-[#94A3B8] hover:text-[#1E293B] '
 }`}
 >
 <Icon size={15} />
 {tab.label}
 <span className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? tab.badgeActive : 'bg-[#F0F2F5] text-[#94A3B8]'}`}>
 {tab.count}
 </span>
 </button>
 );
 ...
 <div key={label}>
<h2 className="text-sm font-medium text-[#94A3B8] mb-6 uppercase tracking-wide">
...
<CardContent className="p-6">
 ...
 className="flex items-center gap-1 px-1.5 py-0.5 text-xs bg-amber-100 text-amber-700 rounded hover:bg-amber-200 transition-colors"
 ...
 <span className="px-1.5 py-0.5 text-xs bg-[#F0F2F5] text-[#94A3B8] rounded" title="Synced from Jira">
 Jira
 </span>
 )}
 {member.excludedFromCapacity && (
 <span className="px-1.5 py-0.5 text-xs bg-[#DEDFE3] text-[#94A3B8] rounded font-bold uppercase tracking-wide" title="Excluded from capacity calculation">
 Excluded
 </span>
 )}
 </div>
 <p className="text-sm text-[#94A3B8] flex items-center gap-1.5 mt-0.5">
 <span>{countryInfo.flag}</span>
 <span>{countryInfo.name}</span>
 </p>
 {member.email && (
 <p className="text-xs text-[#94A3B8] flex items-center gap-1 mt-0.5 truncate" title={member.email}>
 ...
 return squad ? (
 <span className="px-1.5 py-0 text-xs font-medium bg-[#F0F2F5] text-[#94A3B8] rounded">
 {squad.name}
 </span>
 ) : null;
 })()}
 ...
 return pt ? (
 <span key={ptId} className="px-1.5 py-0 text-xs font-medium bg-[#F0F2F5] text-[#94A3B8] rounded">
 {pt.name}
 </span>
 ) : null;
 })}
 ...
 className="p-1.5 text-[#94A3B8] hover:text-[#1E293B] hover:bg-[#F0F2F5] rounded transition-colors"
 ...
 className="p-1.5 text-[#94A3B8] hover:text-amber-500 hover:bg-[#F0F2F5] rounded transition-colors"
 ...
 className="p-1.5 text-[#94A3B8] hover:text-[#0089DD] hover:bg-[#F0F2F5] rounded transition-colors"
 ...
 className="p-1.5 text-[#94A3B8] hover:text-red-500 hover:bg-[#F0F2F5] rounded transition-colors"
 title="Delete"
 >
 <Trash2 size={14} />
 </button>
 ...
 <span 
 key={skill} 
 className="px-2 py-0.5 text-xs bg-[#F0F2F5] text-[#94A3B8] rounded"
 >
 {skill}
 </span>
 ))}
 ...
 <div
 key={member.id}
 className="grid grid-cols-[24px_1fr_140px_140px_160px_auto] gap-2 items-center px-4 py-2.5 hover:bg-[#F0F2F5] /40 transition-colors group"
 >
 {/* Checkbox */}
 <input
 type="checkbox"
 ...
 className="rounded border-[#94A3B8] text-[#0089DD] focus:ring-[#0089DD] cursor-pointer"
 />
 {/* Name + badges */}
 ...
 {/* Role */}
 <span className="text-sm text-[#94A3B8] truncate">{member.role || <span className="text-[#94A3B8]">—</span>}</span>

 {/* Country */}
 <span className="text-sm text-[#94A3B8] flex items-center gap-1.5">
 <span>{countryInfo.flag}</span>
 <span className="truncate">{countryInfo.name || <span className="text-[#94A3B8]">—</span>}</span>
 </span>
 ...
 <span key={skill} className="px-1.5 py-0 text-[10px] bg-[#F0F2F5] text-[#94A3B8] rounded">{skill}</span>
 ))}
 {memberSkills.length > 2 && (
 <span className="text-[10px] text-[#94A3B8]">+{memberSkills.length - 2}</span>
 )}
 </div>
...
<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 opacity-60">
 ...
 return (
 <div key={contact.id} className="grid grid-cols-[24px_1fr_180px_140px_200px_auto] gap-2 items-center px-4 py-2.5 hover:bg-[#F0F2F5] /40 transition-colors group">
 <input
 type="checkbox"
 checked={selectedIds.has(contact.id)}
 onChange={() => toggleSelect(contact.id)}
 className="rounded border-[#DEDFE3] text-[#0089DD] focus:ring-[#0089DD] cursor-pointer"
 />
 <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-7 h-7 rounded-full bg-[#F0F2F5] text-[#94A3B8] text-[10px] font-bold flex items-center justify-center shrink-0">
 {initials}
 </div>
 <div className="min-w-0">
 ...
 {contact.email && <span className="text-[11px] text-[#94A3B8] truncate block">{contact.email}</span>}
 </div>
 </div>
 <span className="text-sm text-[#94A3B8] truncate">{contact.title ?? contact.department ?? <span className="text-[#94A3B8]">—</span>}</span>
 ...
 const pt = processTeams.find(p => p.id === ptId);
 return pt ? (
                            <span key={ptId} className="px-1.5 py-0 text-[10px] font-medium bg-[#F0F2F5] text-[#94A3B8] rounded">{pt.name}</span>
 ) : null;
 })}
 </div>
 <div className="flex items-center gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => { setEditingContact(contact); setBizFormOpen(true); }} className="p-1.5 text-[#94A3B8] hover:text-[#1E293B] hover:bg-[#F0F2F5] rounded" title="Edit"><Edit2 size={13} /></button>
 <div className="relative">
 <button
 onClick={() => setOpenBizListMenu(openBizListMenu === contact.id ? null : contact.id)}
 className="p-1.5 text-[#94A3B8] hover:text-[#94A3B8] hover:bg-[#F0F2F5] rounded transition-colors"
 title="More actions"
 >
 ...
 <div className="absolute right-0 top-full mt-1 z-20 w-40 rounded-lg border border-[#DEDFE3] bg-white shadow-lg py-1">
 <button
 onClick={() => { updateBusinessContact(contact.id, { archived: true }); setOpenBizListMenu(null); }}
 className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#94A3B8] hover:bg-[#F0F2F5] /60 transition-colors text-left"
 >
 <Archive size={13} className="text-[#94A3B8]" />
 Archive
 </button>
...
const countryInfo = getCountryInfo(member.countryId);
return (
<Card key={member.id} className="hover:border-blue-300 transition-colors">
<CardContent className="p-6">
 ...
 <p className="text-xs text-[#94A3B8] mt-0.5">{member.role || '—'}</p>
 <p className="text-xs text-[#94A3B8] flex items-center gap-1 mt-0.5">{countryInfo.flag} {countryInfo.name}</p>
 {memberSkills.length > 0 && (
 <div className="flex flex-wrap gap-1 mt-1.5">
 {memberSkills.slice(0, 3).map(s => <span key={s} className="px-1.5 py-0 text-[10px] bg-[#F0F2F5] text-[#94A3B8] rounded">{s}</span>)}
 {memberSkills.length > 3 && <span className="text-[10px] text-[#94A3B8]">+{memberSkills.length - 3}</span>}
 </div>
 )}
 </div>
 <button onClick={() => handleEdit(member)} className="p-1.5 text-[#94A3B8] hover:text-[#0089DD] hover:bg-[#F0F2F5] rounded"><Edit2 size={13} /></button>
 </div>
 </CardContent>
 </Card>
 );
 ...
 return (
 <div key={m.id} className="grid grid-cols-[24px_1fr_140px_140px_160px_auto] gap-2 items-center px-4 py-2.5 hover:bg-[#F0F2F5] /40 transition-colors group">
 <input type="checkbox" checked={selectedIds.has(m.id)} onChange={() => toggleSelect(m.id)} className="rounded border-[#94A3B8] text-[#0089DD] focus:ring-[#0089DD] cursor-pointer" />
 <div className="flex items-center gap-2.5 min-w-0">
 ...
 <span className="text-sm text-[#94A3B8] flex items-center gap-1"><span>{ci.flag}</span><span className="truncate">{ci.name}</span></span>
 <div className="flex flex-wrap gap-1">
 {(m.processTeamIds ?? []).map(ptId => {
 const pt = processTeams.find(p => p.id === ptId);
 return pt ? <span key={ptId} className="px-1.5 py-0 text-[10px] bg-[#F0F2F5] text-[#94A3B8] rounded">{pt.name}</span> : null;
 })}
 </div>
 <div className="flex gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
 <button onClick={() => handleEdit(m)} className="p-1.5 text-[#94A3B8] hover:text-[#0089DD] rounded"><Edit2 size={13} /></button>
 </div>
 </div>
 );
 })}
 ...
 <div className="min-w-0">
 <span className="text-sm font-medium text-[#1E293B] truncate block">{c.name}</span>
                  <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-[#F0F2F5] text-[#94A3B8] border border-[#DEDFE3]">BIZ</span>
 </div>
 </div>
 <span className="text-sm text-[#94A3B8] truncate">{c.title ?? c.department ?? '—'}</span>
 <span className="text-sm text-[#94A3B8] flex items-center gap-1"><span>{ci.flag}</span><span className="truncate">{ci.name}</span></span>
 <div className="flex flex-wrap gap-1">
 ...
 <div className="flex gap-0.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => { setEditingContact(c); setBizFormOpen(true); }} className="p-1.5 text-[#94A3B8] hover:text-[#1E293B] rounded"><Edit2 size={13} /></button>
 </div>
 </div>
 );
 })}
...
function BizContactCard({
 ...
 return (
                <Card className={`hover:border-[#DEDFE3] transition-colors ${isArchived ? 'opacity-70' : ''}`}>
<CardContent className="p-6">
 <div className="flex items-start justify-between mb-3">
 <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-9 h-9 rounded-full bg-[#F0F2F5] border border-[#DEDFE3] flex items-center justify-center text-sm font-bold text-[#94A3B8] shrink-0">
 {initials}
 </div>
 <div className="min-w-0">
 <div className="flex items-center gap-1.5">
 <span className="text-sm font-semibold text-[#1E293B] truncate">{contact.name}</span>
                      <span className="text-[9px] font-bold tracking-wide uppercase px-1 py-0.5 rounded bg-[#F0F2F5] text-[#94A3B8] border border-[#DEDFE3] shrink-0">BIZ</span>
 {contact.excludedFromCapacity && (
 <span className="text-[9px] font-bold tracking-wide uppercase px-1 py-0.5 rounded bg-[#DEDFE3] text-[#94A3B8] shrink-0">Excluded</span>
 )}
 </div>
 {(contact.title || contact.department) && (
 <p className="text-xs text-[#94A3B8] truncate">{contact.title ?? contact.department}</p>
 )}
 {contact.email && (
 <p className="text-xs text-[#94A3B8] flex items-center gap-1 truncate mt-0.5">
 ...
 className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#94A3B8] hover:bg-[#F0F2F5] /60 transition-colors text-left"
 >
 {isArchived ? <ArchiveRestore size={13} className="text-[#94A3B8]" /> : <Archive size={13} className="text-[#94A3B8]" />}
 ...
 const pt = processTeams.find(p => p.id === ptId);
 return pt ? (
                            <span key={ptId} className="px-1.5 py-0 text-[10px] font-medium bg-[#F0F2F5] text-[#94A3B8] border border-[#DEDFE3] rounded">
 {pt.name}
 </span>
 ) : null;
 })}
 ...
 : pct > 0 ? 'bg-green-50 text-[#16A34A]'
 : 'bg-[#F0F2F5] /40 text-[#94A3B8]'
...
function MassUpdateModal({
 ...
 const fieldClass = "w-full rounded-lg border border-[#94A3B8] bg-white px-3 py-2 text-sm text-[#1E293B] focus:outline-none focus:ring-2 focus:ring-[#0089DD]";
 const labelClass = "block text-xs font-semibold uppercase tracking-wide text-[#94A3B8] mb-1";
 ...
 active ? 'bg-blue-600 border-blue-600 text-white' : 'border-[#94A3B8] text-[#94A3B8] hover:border-[#0089DD]'
 }`}
 >{pt.name}</button>
 );
 })}
...
function ConvertToBizModal({
 ...
 const fieldClass = "w-full rounded-lg border border-[#DEDFE3] bg-white px-3 py-2 text-sm text-[#1E293B] focus:outline-none focus:ring-2 focus:ring-[#0089DD]";
 const labelClass = "block text-xs font-semibold uppercase tracking-wide text-[#94A3B8] mb-1";
 ...
 onClick={handleConfirm}
                className="bg-[#94A3B8] hover:bg-[#1E293B] text-white"
 >
 ...
 {step === 1 ? (
 <div className="space-y-4">
                <div className="flex items-center gap-2 p-3 bg-[#F0F2F5] border border-[#DEDFE3] rounded-lg text-xs text-[#1E293B]">
 ...
 className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-colors ${
                active ? 'bg-[#0089DD] border-[#0089DD] text-white' : 'border-[#DEDFE3] text-[#94A3B8] hover:border-[#0089DD]'
 }`}
 >{pt.name}</button>
 );
 })}
 ...
 <span className="text-xs text-[#94A3B8]">The conversion will proceed without migrating any assignments.</span>
 </div>
 ) : (
 <>
 ...
 <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-[#94A3B8] uppercase tracking-wide">
 <input
 type="checkbox"
 checked={checkedKeys.size === assignedItems.length}
 onChange={toggleAll}
                className="rounded border-[#DEDFE3] text-[#0089DD] focus:ring-[#0089DD]"
              />
 {checkedKeys.size === assignedItems.length ? 'Deselect all' : 'Select all'} ({assignedItems.length})
 </label>
 <span className="text-xs text-[#94A3B8]">{checkedKeys.size} selected</span>
 </div>
 <div className="max-h-64 overflow-y-auto space-y-1 pr-1">
 {assignedItems.map(item => (
 <label key={item.jiraKey} className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-[#F0F2F5] /40 cursor-pointer group">
 <input
 type="checkbox"
 checked={checkedKeys.has(item.jiraKey)}
 onChange={() => toggleKey(item.jiraKey)}
                className="rounded border-[#DEDFE3] text-[#0089DD] focus:ring-[#0089DD] shrink-0"
 />
 <div className="min-w-0 flex-1">
 <div className="flex items-center gap-1.5">
 <span className="text-[10px] font-mono font-semibold text-[#94A3B8] shrink-0">{item.jiraKey}</span>
 <span className="text-xs text-[#94A3B8] uppercase tracking-wide shrink-0">{item.typeName}</span>
 </div>
 <span className="text-sm text-[#1E293B] truncate block">{item.summary}</span>
 </div>
 </label>
 ))}
 </div>
...
function BizContactFormModal({
 ...
 className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-colors ${
                ...
                : 'border-[#DEDFE3] text-[#94A3B8] hover:border-[#0089DD]'
 }`}
 >
 {pt.name}
 </button>
 ...
 <p className="text-xs text-[#94A3B8] mt-0.5">Contact is still visible but not counted in capacity totals</p>
 </div>
 <input
 type="checkbox"
 checked={excludedFromCapacity}
 onChange={e => setExcludedFromCapacity(e.target.checked)}
                className="w-4 h-4 rounded border-[#DEDFE3] text-[#0089DD] focus:ring-[#0089DD] cursor-pointer"
 />
 </label>
 </div>
 </Modal>
