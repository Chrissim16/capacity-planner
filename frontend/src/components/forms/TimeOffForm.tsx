...
export function TimeOffForm({ isOpen, onClose, memberId }: TimeOffFormProps) {
 ...
 {entry.note && (
 <p className="text-xs text-[#94A3B8] truncate">{entry.note}</p>
 )}
 </div>
 </div>
 <button
 onClick={() => removeTimeOff(entry.id)}
 className="ml-2 p-1 text-[#94A3B8] hover:text-red-500 hover:bg-red-50 rounded transition-colors shrink-0"
 title="Remove"
 >
 <Trash2 size={13} />
 </button>
